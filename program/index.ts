...
> = T extends `${infer L}-${infer R}`
  ? CamelCase<R, `${K}${Capitalize<L>}`>
  : `${K}${Capitalize<T>}`;
type ComponentEmitsType<T> = {
  [key in keyof T as `on${key extends string
    ? CamelCase<key>
    : ""}`]?: T[key] extends (...args: infer A) => any
    ? (...args: A) => void
    : T[key];
};

type Types = ComponentEmitsType<Emits>;
// 转化为类型
/*
{
...
// type A<T> = T extends string ? infer T : T;
